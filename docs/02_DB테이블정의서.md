- # HobbyMate DB 테이블 정의서
  ...
  - 회원 탈퇴 시 회원 데이터는 보관하며 CI_HASH만 NULL 처리한다.
  ...
  | `WITHDRAWN` | 회원 탈퇴(회원 정보는 보관하며 CI_HASH는 NULL 처리) |
  ...
  | `PHONE`                | VARCHAR(30)     |    N |                   | 본인인증 휴대폰 번호(회원 탈퇴 후에도 보관)                  |
  ...
  | `MEMBER_STATUS`        | VARCHAR(20)     |    N | `ACTIVE`          | `ACTIVE`, `SUSPENDED`, `WITHDRAWN`<br />WITHDRAWN 회원은 CI_HASH를 NULL 처리하여 즉시 재가입 할 수 있다. |
  ...
  19. WITHDRAWN 회원은 CI_HASH가 NULL이므로 즉시 재가입할 수 있다.
  ...
  - 주민등록번호는 저장하지 않는다.
  - 휴대폰 번호는 암호화하여 저장하며 회원 탈퇴 후에도 보관한다. 화면에는 마스킹하여 표시한다.
